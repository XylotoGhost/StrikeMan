...
// the frontend as window.go.app.App.<Method>.
//
...
type App struct {
	ctx     context.Context
	version string // stamped in by the release build, "dev" otherwise
	...
	workshop    []steam.WorkshopMap // last fetched workshop list, for tag lookups
	lastEnforce map[string]time.Time
}

// New builds the application. version is what the updater compares against.
func New(version string) *App {
	if version == "" {
		version = update.DevVersion
	}
	return &App{version: version, lastEnforce: map[string]time.Time{}}
}

// Startup and Shutdown are the Wails lifecycle hooks.
func (a *App) Startup(ctx context.Context) {
	a.ctx = ctx
	update.CleanupOld() // left behind by a previous self-update
	cfg, err := config.Load()
	if err != nil {
...
func (a *App) connectActive() {
	...
	if s := a.config.ServerByName(a.active); s != nil {
		a.rcon = rcon.New(s.Host, s.Port, s.Password)
	}
}

// server returns a copy of the active server's settings.
func (a *App) server() (config.Server, bool) {
	a.mu.Lock()
	defer a.mu.Unlock()
	if s := a.config.ServerByName(a.active); s != nil {
		return *s, true
...
func (a *App) GetServerInfo() ServerInfo {
	...
	if info.Build > 0 {
		upToDate, latest, note, err := steam.CheckServerUpToDate(info.Build)
		if err != nil {
